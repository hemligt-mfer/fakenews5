...
export default function SidebarCard({
    id, title, image, category, author, createdAt,
}: SidebarCardProps) {
    const authorName = author[0]?.alias ?? null;
        ...
        <Link href={`/article/${id}`} className="group block border-b border-border pb-5 last:border-0">
            ...
            <div className="flex flex-wrap gap-2 mb-1">
                {category.map((c) => (
                    <span
                        key={c.name}
                        className="font-sans text-[10px] font-bold uppercase tracking-[0.08em] text-[#c8a84b]"
                    >
                        {c.name}
                    </span>
                ))}
