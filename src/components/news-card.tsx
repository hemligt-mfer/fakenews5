...
export default function NewsCard({
    ...
    const titleSize =
        size === "hero"  ? "text-2xl leading-tight" :
        size === "small" ? "text-sm leading-snug" :
        size === "text"  ? "text-lg leading-snug" :
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-2 text-xs text-muted-foreground">
                    ...
                    {wasUpdated && (
                        <span className="flex items-center gap-1 text-[#c8a84b]">
                            <RefreshCw size={11} />
                            Updated {format(updatedAt, "d MMM yyyy, HH:mm")}
                        </span>
                    )}
                    {location && (
                        <span className="flex items-center gap-1">
