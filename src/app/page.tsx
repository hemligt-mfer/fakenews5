            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_1px_260px] gap-0 mb-6">
                    ...
                    {hero ? (
                        <Link href={`/article/${hero.id}`} className="group block">
                            <div className="relative w-full aspect-video mb-2.5 overflow-hidden bg-muted border border-border">
                                {hero.image ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                        <span className="font-sans text-[12px] text-muted-foreground">FEATURED IMAGE</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <p className="font-sans text-[11px] text-muted-foreground mt-2">
                                {hero.author.length > 0 && `By ${hero.author.map((a) => a.alias).join(", ")} · `}
                                {formatDistanceToNow(hero.createdAt, { addSuffix: true })}
                            </p>
                        </Link>
                    ) : (
                        <p className="text-muted-foreground text-sm">No editor&apos;s choice set yet.</p>
                    )}
                </div>
                ...
                {rightColumn.length > 0 && (
                    <div className="flex flex-col mt-6 md:mt-0 border-t md:border-t-0 border-border pt-5 md:pt-0">
                        {rightColumn.map((a) => (
                            <SidebarCard key={a.id} id={a.id} title={a.title} image={a.image}
                                category={a.category} author={a.author} createdAt={a.createdAt} />
                        ))}
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-0">
                        {latestNews.length === 0 ? (
                            <p className="text-sm text-muted-foreground col-span-3">No articles published yet.</p>
                        ) : latestNews.map((a, i) => (
                            <div key={a.id} className={`px-0 sm:px-5 first:pl-0 last:pr-0 py-4 sm:py-0 border-b border-border sm:border-b-0 last:border-b-0 ${i < latestNews.length - 1 ? "md:border-r md:border-border" : ""}`}>
                                <NewsCard id={a.id} title={a.title} summary={a.summary} location={a.location}
                                    author={a.author} category={a.category} image={a.image}
                                    createdAt={a.createdAt} updatedAt={a.updatedAt} size="medium" />
                            </div>
                        ))}
                    </div>
                    ...
                    {popularGrid.length > 0 && (
                        <>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-0">
                                {popularGrid.map((a, i) => (
                                    <div key={a.id} className={`px-0 sm:px-5 first:pl-0 last:pr-0 py-4 sm:py-0 border-b border-border sm:border-b-0 last:border-b-0 ${i < popularGrid.length - 1 ? "md:border-r md:border-border" : ""}`}>
                                        <NewsCard id={a.id} title={a.title} summary={a.summary} location={a.location}
                                            author={a.author} category={a.category} image={a.image}
                                            createdAt={a.createdAt} updatedAt={a.updatedAt} size="text" />
                                    </div>
                                ))}
                            </div>
                        </>
